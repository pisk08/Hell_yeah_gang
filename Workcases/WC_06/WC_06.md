...
Було змінено паролі користувачів за допомогою команди:
sudo passwd

<img src="img/05.png">

Також було продемонстровано помилки при неправильному введенні пароля, що підтверджує механізм перевірки безпеки системи.

Далі було налаштовано оболонки (shell) для користувачів за допомогою команди:
sudo usermod -s

Було встановлено різні типи оболонок (bash, zsh, fish), а також обмежено доступ для деяких користувачів шляхом встановлення /usr/sbin/nologin.

<img src="img/06.png">

Після цього було виконано вхід у систему від імені різних користувачів за допомогою команди:
su -

<img src="img/07.png">

У процесі роботи було виконано базові команди для збору інформації про систему, зокрема:

pwd – визначення поточного каталогу;
date – відображення системної дати і часу;
uname -a – отримання інформації про систему.

Було встановлено, що користувачі з різними оболонками мають різне середовище роботи. Наприклад, користувач vice-president використовує оболонку fish, яка має інший інтерфейс у порівнянні з bash.

Також було показано, що користувачі з оболонкою nologin не можуть увійти в систему, що підтверджує обмеження доступу.

Таким чином, було продемонстровано роботу різних груп користувачів, їх налаштування та виконання базових системних команд у командному інтерпретаторі.
